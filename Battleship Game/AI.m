...
-(NSMutableArray*) Search
{
    ...
    //[[Rows objectAtIndex:r] remo];
    NSMutableArray *temp = [[NSMutableArray alloc] init];
    temp = [self CheckEligibility:c :r];
    if (Map [(r * 10) + c] > 1)
    {
        [temp replaceObjectAtIndex:0 withObject:[NSNumber numberWithBool:false]];//=false;
        [temp replaceObjectAtIndex:1 withObject:[NSNumber numberWithBool:false]];//=false;
    }
...
-(NSMutableArray*) Destroy
{
    ...
    int c = HitPoint%10; int r = HitPoint/10;
    NSMutableArray *eligible = [self CheckEligibility:c :r];
    bool h = [[eligible objectAtIndex:0] boolValue]; bool v = [[eligible objectAtIndex:1] boolValue];
    int tempcol = c; int temprow = r;
    
    int up = ((r+1)*10)+c; int down = ((r-1)*10)+c; int left = (r*10)+c-1; int right = (r*10)+c+1;
    ...
    if(r==9)[u1 replaceObjectAtIndex:0 withObject:[NSNumber numberWithBool:false]];//= false;
    if(r==0)[d1 replaceObjectAtIndex:0 withObject:[NSNumber numberWithBool:false]];//= false;
    if(c==0)[l1 replaceObjectAtIndex:0 withObject:[NSNumber numberWithBool:false]];//= false;
    if(c==9)[r1 replaceObjectAtIndex:0 withObject:[NSNumber numberWithBool:false]];//= false;
    ...
    int nextup=-1;int nextdown=-1;int nextleft=-1;int nextright=-1;
